...
export default function SelectorMedicamentosReceta({ receta, setReceta }) {
  ...
  const [detalle, setDetalle] = useState({ dosis: "", frecuencia: "", duracion: "", observaciones: "" });
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`${BASE_URL}api_medicamentos.php?busqueda=${encodeURIComponent(busqueda)}`)
      .then(res => res.json())
    ...
    <div className="mb-4">
      ...
      {medicamentoSel && (
        <div className="border rounded p-2 bg-blue-50 mb-2">
          <div className="font-semibold mb-1">{medicamentoSel.nombre} <span className="text-gray-500">({medicamentoSel.codigo})</span></div>
          <div className="flex flex-col sm:flex-row gap-2 mb-1">
            <input
              ...
              value={detalle.dosis}
              onChange={e => setDetalle(d => ({ ...d, dosis: e.target.value }))}
            />
            <input
              type="text"
              className="border rounded p-1 flex-1"
              ...
              onChange={e => setDetalle(d => ({ ...d, frecuencia: e.target.value }))}
            />
            <input
              type="text"
              className="border rounded p-1 flex-1"
              ...
              onChange={e => setDetalle(d => ({ ...d, duracion: e.target.value }))}
            />
          </div>
          <textarea
          ...
          <div className="flex gap-2">
            <button type="button" className="bg-blue-600 text-white px-3 py-1 rounded" onClick={agregarMedicamento}>
              Agregar a receta
            </button>
            <button type="button" className="bg-gray-400 text-white px-3 py-1 rounded" onClick={() => setMedicamentoSel(null)}>
              Cancelar
            </button>
          </div>
        </div>
          ...
          <tbody>
            {recetaArray.length > 0 ? (
              recetaArray.map((m, idx) => (
                <tr key={m.codigo + idx}>
                  <td className="border px-2 py-1">{m.nombre} <span className="text-gray-500">({m.codigo})</span></td>
                  ...
                  <td className="border px-2 py-1 text-center">
                    <button type="button" className="text-red-600 font-bold" onClick={() => eliminarMedicamento(m.codigo)}>
                      X
                    </button>
                  </td>
                </tr>
