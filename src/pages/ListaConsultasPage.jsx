...
export default function ListaConsultasPage() {
  ...
  // Exportar a Excel
  const exportarExcel = () => {
    const ws = XLSX.utils.json_to_sheet(consultasFiltradas.map(c => ({
      ID: c.id,
  ...
  return (
    <div className="max-w-5xl mx-auto p-4">
      <h1 className="text-2xl font-bold mb-6 text-center">Lista de Consultas</h1>
  <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {(fechaDesde || fechaHasta || busqueda) && (
          <button onClick={() => { setFechaDesde(""); setFechaHasta(""); setBusqueda(""); }} className="text-blue-600 underline ml-2">Limpiar filtro</button>
        )}
        <button onClick={exportarExcel} className="bg-green-600 text-white px-3 py-1 rounded font-semibold text-xs hover:bg-green-700 transition-all">Exportar Excel</button>
        <button onClick={exportarPDF} className="bg-red-600 text-white px-3 py-1 rounded font-semibold text-xs hover:bg-red-700 transition-all">Exportar PDF</button>
      </div>
      {loading ? (
          ...
          <table className="min-w-full text-xs md:text-sm border bg-white rounded shadow">
            ...
            <tbody>
              {consultasFiltradas.length === 0 ? (
                <tr><td colSpan={6} className="text-center p-4">No hay consultas registradas</td></tr>
              ) : (
                pagedConsultas.map(c => (
                  <tr key={c.id} className="border-b hover:bg-blue-50">
                    <td className="p-2 w-12 text-center">{c.id}</td>
                    <td className="p-2">{c.fecha?.slice(0,16).replace("T"," ")}</td>
        ...
        <div className="flex justify-end items-center gap-2 mt-2">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-2 py-1 rounded bg-gray-200 disabled:opacity-50">Anterior</button>
          <span className="text-sm">Página {page} de {totalPages}</span>
          <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-2 py-1 rounded bg-gray-200 disabled:opacity-50">Siguiente</button>
        </div>
            </tbody>
          </table>
        </div>
